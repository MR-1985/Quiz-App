...
let rightAnswer = animalQuestions['right_answer'];
...
function showCountOfAllQuestions() {
    ...
    allQuestionsRef.innerHTML = animalQuestions.length;
};

function showQuestion() {
    let question = animalQuestions[currentQuestion];
    let questionRef = document.getElementById("question")
    questionRef.innerText = question['question']

    let percent = (currentQuestion + 1) / animalQuestions.length;
...
function showQuizResult(){
    ...
    possibleAnswers.innerHTML = animalQuestions.length;
};
