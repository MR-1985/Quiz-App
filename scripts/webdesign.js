...
let rightAnswer = webdesignQuestions['right_answer'];
...
function showCountOfAllQuestions() {
    ...
    allQuestionsRef.innerHTML = webdesignQuestions.length;
};

function showQuestion() {
    let question = webdesignQuestions[currentQuestion];
    let questionRef = document.getElementById("question")
    questionRef.innerText = question['question']

    let percent = (currentQuestion + 1) / webdesignQuestions.length;
...
function selectAnswer(selectedAnswer) {
    ...
    if (currentQuestion === webdesignQuestions.length - 1) {
...
function showQuizResult(){
    ...
    possibleAnswers.innerHTML = webdesignQuestions.length;
};
